...
import {
  Palette, Sparkles, Image as ImageIcon, Frame,
  Check, Download, ShieldCheck, RefreshCw
} from 'lucide-react';
...
const PRESET_PALETTES = [
  { id: 'obsidian', name: 'Obsidian', fg: '#0f172a', bg: '#ffffff', gradStart: '#0f172a', gradEnd: '#334155' },
  ...
  { id: 'gold', name: 'Amber Gold', fg: '#d97706', bg: '#ffffff', gradStart: '#b45309', gradEnd: '#f59e0b' },
  { id: 'dark-glass', name: 'Dark Slate', fg: '#ffffff', bg: '#090d16', gradStart: '#ffffff', gradEnd: '#cbd5e1' },
];
...
export function QRDesignStudio({
  ...
  return (
    <div className={cn('grid lg:grid-cols-[1.1fr_0.9fr] gap-6 items-start', className)}>
      {/* Controls Column */}
      <div className="surface rounded-3xl p-5 sm:p-7 border border-slate-200/80 dark:border-white/10 space-y-6">
        {/* Navigation Tabs Bar */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 p-1 rounded-2xl bg-slate-900/[0.04] dark:bg-white/[0.04]">
          {[
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl text-xs font-semibold transition-all cursor-pointer text-center',
                  active
        ...
        {activeTab === 'colors' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {PRESET_PALETTES.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className="w-5 h-5 rounded-full flex-shrink-0 shadow-sm ring-1 ring-black/10 dark:ring-white/10"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-slate-200/50 dark:border-white/5">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-10 h-10 rounded-xl border border-slate-300 dark:border-white/15 cursor-pointer bg-transparent"
                  />
                  <input
                    type="text"
                    value={styleConfig.fg_color || '#0f172a'}
                    onChange={(e) => updateConfig({ fg_color: e.target.value })}
                    className="flex-1 px-3 py-2 text-xs font-mono rounded-xl bg-slate-900/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 uppercase"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-10 h-10 rounded-xl border border-slate-300 dark:border-white/15 cursor-pointer bg-transparent disabled:opacity-30"
                  />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                ...
                {PRESET_LOGOS.map((l) => (
                  ...
                  >
                    {l.svg ? (
                      ...
                      <span className="w-4 h-4 flex items-center justify-center font-bold">{l.icon}</span>
                    )}
                    <span>{l.label}</span>
                  </button>
                ))}
            ...
            <div className="pt-3 border-t border-slate-200/50 dark:border-white/5">
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl text-xs bg-slate-900/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 focus:border-indigo-500 focus:outline-none"
              />
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1.5 flex items-center gap-1.5">
                <ShieldCheck size={14} className="text-emerald-500 flex-shrink-0" />
                <span>Level-H error correction protects up to 30% scannability.</span>
              </p>
            </div>
          </motion.div>
        )}
              ...
              <div className="space-y-3 pt-2 border-t border-slate-200/50 dark:border-white/5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl text-xs bg-slate-900/5 dark:bg-white/5 border border-slate-200 dark:border-white/10 uppercase tracking-wider font-bold"
                  />
                </div>
              </div>
            )}
      ...
      <div className="surface rounded-3xl p-5 sm:p-7 border border-slate-200/80 dark:border-white/10 flex flex-col items-center justify-between text-center sticky top-24">
        ...
        <div className="relative flex flex-col items-center p-5 bg-white dark:bg-slate-900 rounded-3xl shadow-xl ring-1 ring-slate-900/10 dark:ring-white/10 my-auto">
          {/* Top Frame Banner */}
          ...
          {/* SVG QR Code */}
          <div
            className="w-44 h-44 sm:w-52 sm:h-52 relative flex items-center justify-center"
